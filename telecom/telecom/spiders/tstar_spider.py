...
import scrapy
from telecom.items import TstarStoreItem
import json
import random
import re

class TSTAR_Spider(scrapy.Spider):
            ...
            for i in range(0, len(cities)):
                ...
                for j in range(0, len(areas[i])):
                    ...
                    yield scrapy.Request(
                        'https://www.tstartel.com/rest/serviceLocation/'+cities[i]+areas[i][j],
                        self.parse,
                        method='GET'
                    )
    ...
    def parse(self, response):
        ...
        for i in range(len(json_response)):
            item = TstarStoreItem()
            if (json_response[i]['storeType']=='0'):
                item['storeType'] = '直營'
            else:
                item['storeType'] = '特約'
